...
var micromatch$1 = /*@__PURE__*/getDefaultExportFromCjs(micromatch_1);

class PluginManager {
    // Clean directories that match the condition
    static cleanDir(sourceDir, targetDir) {
    ...
    static copyDistFolders(sourceDir, targetDir) {
        ...
        distDirs.forEach((dir) => {
            ...
            if (normalizedDir) {
                const fullSourcePath = path$2.join(sourceDir, normalizedDir);
    ...
    static moveFiles(sourceDir, targetDir, pattern = '**/*', matchOption) {
        const files = PluginManager.getAllFiles(sourceDir, pattern, [], matchOption);
        files.forEach((file) => {
            const relativePath = path$2.relative(sourceDir, file);
            const destination = path$2.join(targetDir, relativePath);
            const destinationDir = path$2.dirname(destination);
    ...
    static clearUnnecessaryFiles(targetDir, patterns) {
        const files = PluginManager.getAllFiles(targetDir, '**/*');
        files.forEach((file) => {
            const matchesEveryPattern = micromatch$1.some(file, patterns);
    ...
    static clearEmptyDirs(directory) {
        ...
        fileStats.forEach((fileStat) => {
            const fullPath = path$2.join(directory, fileStat.name);
     ...
     */
    static findDistDirs(dir, getRealPath = false) {
        const result = [];
        const items = fs.readdirSync(dir, { withFileTypes: true });
        for (const item of items) {
            const fullPath = path$2.join(dir, item.name);
            if (item.isDirectory()) {
                if (item.name === 'dist') {
     ...
     */
    static findDestDirs(sourceDir, targetDir) {
        const results = [];
        const entries = fs.readdirSync(sourceDir, { withFileTypes: true });
        entries.forEach((entry) => {
            const fullPath = path$2.join(targetDir || sourceDir, entry.name);
    ...
    static copyRecursiveSync(src, dest) {
        ...
        if (fs.existsSync(src) && fs.lstatSync(src).isDirectory()) {
            ...
            for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
                const srcPath = path$2.join(src, entry.name);
                const destPath = path$2.join(dest, entry.name);
    ...
    static getAllFiles(dir, pattern, fileList = [], matchOption) {
        const files = fs.readdirSync(dir);
        files.forEach((file) => {
            const fullPath = path$2.join(dir, file);
            const stat = fs.statSync(fullPath);
            if (stat.isDirectory()) {
                fileList = PluginManager.getAllFiles(fullPath, pattern, fileList, matchOption);
            }
            else if (stat.isFile() && micromatch$1.isMatch(fullPath, pattern, matchOption)) {
